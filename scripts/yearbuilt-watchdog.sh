#!/bin/bash
LOGFILE="/home/dentwon/Eavesight/logs/yearbuilt-v3.log"
CHECK_INTERVAL=300

echo "$(date): Watchdog started" >> ${LOGFILE}.watchdog

while true; do
    if ! pgrep -f "enrich-yearbuilt-v3" > /dev/null; then
        ...
        if [ "$HTTP_CODE" = "200" ]; then
            ...
            node scripts/enrich-yearbuilt-v3.js >> $LOGFILE 2>&1 &
